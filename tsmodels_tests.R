...
#below will use 4 cores on a multicore computer)
library(doParallel)
cl <- makeCluster(8)  # Use 4 cores
...
pvisits <- DF %>% dplyr::select(ds,y=visits) %>% prophet(holidays = holidays, 
                                                         ...
                                                         weekly.seasonality = weekly.seasonality,
                                                         daily.seasonality = daily.seasonality,
                                                         seasonality.mode = "multiplicative") %>% predict(pfdat)
...
str(ts_visits)
visitsAlphaBetaGamma <- HoltWinters(ts_visits, seasonal = "mult")
...
# -------------------------------------------
# -------------------------------------------
## Auto.Arima w/ Seasonal order
# takes in 'rnb' from above which is a 'ts' object
model_sorder <- auto.arima(rnb, D=1) # takes in a ts(object) as input data and outputs a sarima model
asorder <- model_sorder$arma[c(1, 6, 2, 3, 7, 4, 5)]
names(asorder) <- c("p", "d", "q", "P", "D", "Q", "Frequency")
model_fit_s <- stlm(rnb, modelfunction=Arima,
                    order=c(asorder[1],asorder[2],asorder[3]),
                    seasonal=list(order=c(asorder[4],asorder[5],asorder[6])))
fit_sarima_fc <- forecast(model_fit_s, h=h)
write.csv(fit_sarima_fc, "7_rnb_sarima_pred_fc.csv")
# -------------------------------------------
# -------------------------------------------
# Compute MAPES
df_mape <- data.frame(rnb, fit_arima_fc$fitted, fit_hw_fc$fitted, fit_sarima_fc$fitted)
mape_arima <- mean(abs((df_mape$rnb - df_mape$fit_arima_fc.fitted)/df_mape$rnb))
mape_arima
mape_sarima <- mean(abs((df_mape$rnb - df_mape$fit_sarima_fc.fitted)/df_mape$rnb))
mape_sarima
...
mapes_by_model_Hols <- data_frame(
  ...
  mape = c(mape_prophet,mape_prophet_visits,
           mape_hw,mape_arima,mape_sarima) 
)
